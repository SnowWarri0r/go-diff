...
// Lovart fork extension: rune-based Match — mirrors MatchMain/MatchBitap/MatchAlphabet
// but operates on []rune instead of UTF-8 bytes, so position/length math aligns
// with Google JS DMP (UTF-16 code unit ≈ rune for BMP characters).
//
// Motivation: sergi byte-based Bitap on multi-byte text (中文 / em dash / emoji)
// uses len(text) (byte count), patches[].Length1 (byte count), substring slice by
// byte index. Same algorithm on JS uses UTF-16 unit count. Same input patch text
// (whose Length/Start numbers were emitted by JS-side patch_make as UTF-16 units)
// applied via byte-mode sergi causes:
//   - PatchSplitMax 切段过碎 (byte length 增长 ~3x 比 unit length)
//   - expected_loc = Start2 + delta drifts by N bytes for N multi-byte chars
//   - Bitap pattern shorter → fuzzy match accuracy weakens
//   - cascading 422 / silent imperfect match
//
// rune mirror keeps algorithm structure 1:1 with sergi original; only length
// and indexing model differs.
