...
// Lovart fork extension: rune-based Patch{Apply,SplitMax,AddPadding} +
// helpers — mirrors original Patch* but counts length / slices by rune index
// instead of UTF-8 byte. See match_runes.go header for motivation.
//
// API surface:
//   - PatchApplyRunes(patches, text) ([]rune, []bool)  ← new public entry point;
//     callers convert their string to []rune before calling and string() the
//     returned []rune. Patch.Length1/Length2/Start1/Start2 are interpreted as
//     rune counts (Google JS DMP wire format == UTF-16 unit ≈ rune for BMP).
//   - The original PatchApply (byte-based) is preserved with rune-safe slicing
//     guards (panic防护) for callers that still want byte-mode behavior.
...
//
// Behavior parity with sergi PatchApply preserved at structure level; the
// difference is: all length comparisons, slice indices, MatchMain calls, and
// substring extractions operate in rune space — eliminating the byte/UTF-16
// unit mismatch that causes silent fuzzy drift on multi-byte text.
//
// fork safety net: defer recover() converts residual panic into
// ([] for text, [false x N]) so callers see "all patches failed" instead of
// a process-killing stack trace.
func (dmp *DiffMatchPatch) PatchApplyRunes(patches []Patch, text []rune) (out []rune, results []bool) {
	out = text
	if len(patches) == 0 {
		results = []bool{}
		return
